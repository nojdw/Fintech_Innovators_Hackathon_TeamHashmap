...
const WARN_THRESHOLD  = 0.50; // single bucket > 50% → warning
const ALERT_THRESHOLD = 0.70; // single bucket > 70% → alert
...
function fmtCcy(v, currency = "USD") {
  const n = parseFloat(v);
  if (isNaN(n)) return "—";
  const sym = currency === "SGD" ? "SGD " : currency === "HKD" ? "HKD " : currency === "GBP" ? "£" : "USD ";
  return sym + n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

// Convert USD value to SGD for the net worth bar
function toSGD(usdVal) { return parseFloat(usdVal) * USD_TO_SGD; }
...
function makeChart(id, type, data, options = {}) {
  ...
  CHARTS[id] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { labels: { color: "#7a90aa", font: { family: "Share Tech Mono", size: 10 }, boxWidth: 10 } },
      },
      scales: (type === "bar" || type === "line") ? {
        x: { ticks: { color: "#7a90aa", font: { family: "Share Tech Mono", size: 9 } }, grid: { color: "#141b28" } },
        y: { ticks: { color: "#7a90aa", font: { family: "Share Tech Mono", size: 9 } }, grid: { color: "#141b28" } },
      } : {},
      ...options,
    },
  });
...
async function load() {
  ...
  try {
    const [sR, aR, pR, tR, cR, mR] = await Promise.all([
      ...
      fetch("/api/transactions"), fetch("/api/bank/DBS"), fetch("/api/bank/SC"),
    ]);
    ...
    renderHealthScore(_allPositions, account, _DBSData, _SCData);
...
function renderNetWorth(account, DBS, SC) {
  ...
  const DBSSGD     = parseFloat(DBS.account.balance || DBS.account.current_balance || 0);
  const SCSGD    = parseFloat(SC.account.balance  || SC.account.current_balance  || 0);
...
function renderBankingCharts(DBS, SC) {
  ...
  makeChart("chart-cashflow-DBS", "bar", {
    labels: DBSMonthly.map(m => m.label),
    datasets: [
      { label: "Income",   data: DBSMonthly.map(m => m.inc), backgroundColor: "rgba(57,255,110,0.15)", borderColor: "#39ff6e", borderWidth: 2 },
      { label: "Expenses", data: DBSMonthly.map(m => m.exp), backgroundColor: "rgba(224,32,32,0.15)",  borderColor: "#e02020", borderWidth: 2 },
    ],
  }, { plugins: { legend: { position: "bottom" } } });

  const SCMonthly = getMonthlyData(SC, months);
  makeChart("chart-cashflow-SC", "bar", {
    labels: SCMonthly.map(m => m.label),
    datasets: [
      { label: "Income",   data: SCMonthly.map(m => m.inc), backgroundColor: "rgba(77,166,255,0.15)", borderColor: "#4da6ff", borderWidth: 2 },
      { label: "Expenses", data: SCMonthly.map(m => m.exp), backgroundColor: "rgba(224,32,32,0.15)",  borderColor: "#e02020", borderWidth: 2 },
    ],
  }, { plugins: { legend: { position: "bottom" } } });
  ...
  function buildPie(filtered, fallbackLabels, fallbackData) {
    ...
    return { labels: keys.length ? keys : fallbackLabels, data: keys.length ? keys.map(k => cats[k].toFixed(2)) : fallbackData };
  }

  const cP = buildPie(filterByTimeline(DBS.expenses), [], []);
  makeChart("chart-expenses-DBS", "doughnut", {
    labels: cP.labels,
    datasets: [{ data: cP.data, backgroundColor: COLORS, borderWidth: 0 }],
  }, { plugins: { legend: { position: "right", labels: { font: { size: 10 } } } } });

  const mP = buildPie(filterByTimeline(SC.expenses), ["Transfers to Brokerage", "Emergency Fund"], [9000, 1500]);
  makeChart("chart-expenses-SC", "doughnut", {
    labels: mP.labels,
...
function renderPositionsTable() {
  ...
  document.getElementById("positions-body").innerHTML = rows.map(p => {
    const pct52 = p.week_52_high > p.week_52_low
      ? (((p.market_price - p.week_52_low) / (p.week_52_high - p.week_52_low)) * 100).toFixed(0) : 50;
...
function filterTx() {
  ...
  if (q) rows = rows.filter(t =>
    t.symbol.toLowerCase().includes(q) || t.action.toLowerCase().includes(q)
  );
...
function renderPortfolio(positions, txs, account) {
  ...
  document.getElementById("portfolio-stats").innerHTML = [
    { label: "PORTFOLIO VALUE (USD)", value: fmtUSD(totalValue),    sub: `${positions.length} POSITIONS — LIVE`,       cls: "" },
    { label: "TOTAL RETURN",          value: fmtPct(totalReturn),   sub: `COST BASIS: ${fmtUSD(totalCost)}`,           cls: pnlClass(totalReturn) },
    { label: "UNREALIZED P&L (USD)",  value: fmtUSD(totalUnreal),   sub: "OPEN POSITIONS",                             cls: pnlClass(totalUnreal) },
    { label: "REALIZED P&L (USD)",    value: fmtUSD(totalReal),     sub: "CLOSED TRADES",                              cls: pnlClass(totalReal) },
...
function computeHealthScore(positions, account, DBS, SC) {
  ...
  const wDiv = divPos.length
    ? divPos.reduce((s, p) => s + p.dividend_yield * p.market_value, 0) / totalValue
    : 0;
  ...
  const avgPE = equities.length ? equities.reduce((s, p) => s + p.pe_ratio, 0) / equities.length : 24;
  ...
  const DBSbal  = parseFloat(DBS.account.balance || DBS.account.current_balance || 0);
  const SCbal = parseFloat(SC.account.balance || SC.account.current_balance || 0);
  ...
  // ── WEIGHTED FINAL ────────────────────────────────────
  const weights = { risk: 0.20, div: 0.25, income: 0.15, val: 0.15, quality: 0.15, liquidity: 0.10 };
  ...
  return {
    total,
    breakdown: [
      { label: "DIVERSIFICATION", score: Math.round(divScore),        weight: "25%", detail: `Avg HHI ${avgHHI.toFixed(2)}` },
      { label: "RISK",            score: Math.round(riskScore),       weight: "20%", detail: `Wtd beta ${wBeta.toFixed(2)}` },
      { label: "INCOME",          score: Math.round(incomeScore),     weight: "15%", detail: `Div yield ${wDiv.toFixed(2)}%` },
      { label: "VALUATION",       score: Math.round(valScore),        weight: "15%", detail: `Avg P/E ${avgPE.toFixed(1)}x` },
      { label: "QUALITY",         score: Math.round(qualityScore),    weight: "15%", detail: `Avg margin ${wMargin.toFixed(1)}%` },
      { label: "LIQUIDITY",       score: Math.round(liquidityScore),  weight: "10%", detail: `Cash ${cashPct.toFixed(1)}% of NW` },
    ],
  };
}
...
function renderHealthScore(positions, account, DBS, SC) {
  ...
  const color = score >= 70 ? "var(--green)" : score >= 45 ? "var(--amber)" : "var(--red2)";
  const grade = score >= 80 ? "A" : score >= 65 ? "B" : score >= 50 ? "C" : score >= 35 ? "D" : "F";
  const label = score >= 70 ? "HEALTHY" : score >= 45 ? "MODERATE RISK" : "NEEDS ATTENTION";

  const breakdownHTML = result.breakdown.map(b => {
    const bColor = b.score >= 70 ? "var(--green)" : b.score >= 45 ? "var(--amber)" : "var(--red2)";
...
function checkConcentration(map, totalValue, label) {
  ...
  Object.entries(map).forEach(([key, val]) => {
    ...
    if (pct > ALERT_THRESHOLD) {
      warnings.push({ level: "alert", text: `${key} represents ${(pct * 100).toFixed(1)}% of ${label}` });
    } else if (pct > WARN_THRESHOLD) {
      warnings.push({ level: "warn", text: `${key} represents ${(pct * 100).toFixed(1)}% of ${label}` });
    }
  });
  return warnings;
}
...
function renderDiversificationWarnings(positions) {
  ...
  const allWarnings = [
    ...
    ...checkConcentration(groupBy("geography"),  totalValue, "GEOGRAPHIC EXPOSURE"),
    ...checkConcentration(groupBy("asset_class"),totalValue, "ASSET CLASS BREAKDOWN"),
  ];
...
function renderAnalysis(positions) {
  ...
  document.getElementById("fundamentals-grid").innerHTML = [
    ...
    { label: "WEIGHTED DIV YIELD",   value: wDiv > 0 ? fmtPct(wDiv * 100) : "—",        sub: `~${fmtUSD(annDiv)}/YR EST.` },
    { label: "POSITIONS PAYING DIV", value: `${divPos.length} / ${positions.length}`,   sub: "PAY DIVIDENDS" },
    { label: "LARGEST POSITION",     value: sortedByVal[0]?.symbol || "—",              sub: fmtUSD(sortedByVal[0]?.market_value) },
  ...
  const rates = [
    ...
    { label: "SC SAVINGS APY",    rate: 5.10, note: "YOUR SAVINGS RATE" },
...
function computeRetirement(p, positions, DBS, SC) {
  ...
  const annualSavings = DBS.monthly_income * 12 - DBS.monthly_expenses * 12 +
  ...
  const fvPortfolio = currentAssets * Math.pow(1 + growthRate, yearsToRetire) +
    annualSavings * (Math.pow(1 + growthRate, yearsToRetire) - 1) / growthRate;
  ...
  const inflatedMonthlyExp = p.monthly_expenses_retirement * Math.pow(1 + inflationRate, yearsToRetire);
  ...
  if (totalAtRetirement > 0 && netAnnualDraw > 0) {
    if (realRate > 0) {
      ...
      if (ratio < 1) yearsCorporusLasts = -Math.log(1 - ratio) / Math.log(1 + realRate);
    } else {
  ...
  const annualDivIncome = positions.reduce((s, pos) => s + (pos.market_value * pos.dividend_yield) / 100, 0);
  const projectedDivAtRetirement = annualDivIncome * Math.pow(1 + growthRate, yearsToRetire);

  // Year-by-year projection for chart (portfolio value only, not CPF)
  const chartYears = [], chartVals = [], chartNeeded = [];
  ...
  return {
    ...
    yearsCorporusLasts: isFinite(yearsCorporusLasts) ? Math.round(yearsCorporusLasts) : 999,
    ...
    chartYears, chartVals, chartNeeded,
  };
}
...
async function loadRetirementProfile() {
  ...
  } catch (e) { console.error("Could not load retirement profile", e); }
}
...
function toggleGoalFields() {
  ...
  if (divRow) divRow.style.display = sel === "dividend_income" ? "flex" : "none";
...
async function saveAndRunRetirement() {
  ...
  try {
    ...
    setTimeout(() => { document.getElementById("r-save-status").textContent = ""; }, 2000);
  } catch (e) {
...
function renderRetirementResults() {
  ...
  const r = computeRetirement(_retirementProfile, _allPositions, _DBSData, _SCData);
  if (!r) return null;
  ...
  const kpis = [
    { label: "PROJECTED CORPUS (SGD)",    value: fmtSGD(r.totalAtRetirement),  sub: `At age ${_retirementProfile.retirement_age}`,    color: "" },
    { label: "CORPUS NEEDED (SGD · 4%)",  value: fmtSGD(r.swrCorpusNeeded),    sub: "Safe withdrawal rate target",                    color: "" },
    { label: "CORPUS GAP (SGD)",          value: r.onTrack ? "SURPLUS " + fmtSGD(Math.abs(r.gap)) : fmtSGD(r.gap), sub: r.onTrack ? "You have a surplus" : "Additional capital needed", color: statusColor },
    { label: "YEARS CORPUS LASTS",        value: r.yearsCorporusLasts >= 999 ? "∞" : r.yearsCorporusLasts + " YRS", sub: `Drawing ${fmtSGD(r.inflatedMonthlyExp)}/mo`, color: r.yearsCorporusLasts >= 30 ? "var(--green)" : "var(--amber)" },
    { label: "ANNUAL SAVINGS (SGD)",      value: fmtSGD(r.annualSavings),      sub: "Current trajectory",                             color: "" },
    { label: "INFLATED MONTHLY EXP (SGD)",value: fmtSGD(r.inflatedMonthlyExp), sub: `In ${r.yearsToRetire}yr at ${_retirementProfile.expected_inflation_rate}% p.a.`, color: "" },
  ];
  ...
  const cpfHTML = _retirementProfile.include_cpf ? `
    ...
    <div class="grid-4" style="margin-bottom:1.25rem">
      ${[
        { label: "OA AT RETIREMENT",  value: fmtSGD(r.cpfOAatRetirement),   sub: "2.5% p.a. growth" },
        { label: "SA AT RETIREMENT",  value: fmtSGD(r.cpfSAatRetirement),   sub: "4.0% p.a. growth" },
        { label: "WITHDRAWAL AT 55",  value: fmtSGD(r.cpfWithdrawalAt55),   sub: "Above FRS" },
        { label: "CPF LIFE PAYOUT",   value: r.cpfLifeMonthly > 0 ? fmtSGD(r.cpfLifeMonthly) + "/mo" : "N/A", sub: "From age 65 est." },
  ...
  panel.innerHTML = `
    ...
    <div class="grid-3" style="margin-bottom:1.25rem">
      ${kpis.map(k => `<div class="stat-card" ${k.color ? `style="border-left-color:${k.color}"` : ""}>
  ...
  setTimeout(() => {
    ...
    makeChart("chart-retirement", "line", {
      labels: r.chartYears,
      datasets: [
        {
          label: "Portfolio Value",
          data: r.chartVals,
          borderColor: "#e8620a", backgroundColor: "rgba(232,98,10,0.08)",
        ...
        },
        {
          label: "Corpus Needed",
          data: r.chartNeeded,
          borderColor: "#3d5068", borderDash: [6, 3],
      ...
      scales: {
        y: { ticks: { callback: v => "S$" + (v >= 1e6 ? (v / 1e6).toFixed(1) + "M" : (v / 1e3).toFixed(0) + "K") } },
      },
    });
  }, 50);
...
async function requestAiAnalysis() {
  ...
  if (_retirementProfile && _allPositions.length && _DBSData) {
    retResult = computeRetirement(_retirementProfile, _allPositions, _DBSData, _SCData);
    _lastRetirementResult = retResult;
  }

  if (!retResult) {
    const panel = document.getElementById("ai-insights-panel");
    if (panel) panel.innerHTML = `<div class="ai-error">⚠ Run the Retirement Calculator first to provide projection context for the AI analysis.</div>`;
    const sh = document.getElementById("ai-insights-sh");
    if (sh) sh.style.display = "";
    if (btn) { btn.disabled = false; btn.textContent = "▶ REQUEST AI ANALYSIS"; }
    return;
  }
...
function renderAiMeter(status) {
  ...
  const color = used >= limit ? "var(--red2)" : used >= limit * 0.67 ? "var(--amber)" : "var(--green)";
...
function buildInsightPayload(healthScore, retirementResult) {
  ...
  return {
    ...
    projected_div_at_retirement:  retirementResult.projectedDivAtRetirement || null,
  };
}
...
function renderAiInsights(data, model) {
  ...
  const obsHTML = data.observations.map(o =>
    `<li class="ai-obs">${o}</li>`
  ).join("");

  const sigHTML = data.rebalance_signals.map(s =>
    `<div class="ai-signal">→ ${s}</div>`
  ).join("");
