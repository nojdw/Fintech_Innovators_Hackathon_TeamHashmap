...
async function load() {
  ...
  try {
    const [sR, aR, pR, tR, cR, mR] = await Promise.all([
      ...
      fetch("/api/bank/chase"),
      fetch("/api/bank/marcus"),
    ]);
...
function renderBankingCharts(chase, marcus) {
  ...
  makeChart(
    ...
    "bar",
    {
      ...
      datasets: [
        {
          label: "Income",
          data: marcusMonthly.map((m) => m.inc),
        ...
        {
          label: "Expenses",
          data: marcusMonthly.map((m) => m.exp),
          backgroundColor: "rgba(224,32,32,0.15)",
          borderColor: "#e02020",
          borderWidth: 2,
        },
  ...
  makeChart(
    "chart-expenses-chase",
    "doughnut",
    {
      labels: ck,
      datasets: [
        {
          data: ck.map((k) => chaseCats[k].toFixed(2)),
      ...
      datasets: [
        {
          data:
            mk.length > 0
              ? mk.map((k) => marcusCats[k].toFixed(2))
              : [9000, 1500],
          backgroundColor: COLORS,
          borderWidth: 0,
        },
...
function renderAnalysis(positions) {
  ...
  document.getElementById("fundamentals-grid").innerHTML = [
    ...
    },
    {
      ...
      sub: fmt(sortedByVal[0]?.market_value),
    },
    {
  ...
  const rates = [
    ...
    { label: "MARCUS HYSA APY", rate: 5.1, note: "YOUR SAVINGS RATE" },
